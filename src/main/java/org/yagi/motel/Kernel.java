...
public class Kernel {
    ...
    private static final String RESULT_COMMAND_DISPATCHER_THREAD_PREFIX = "result-commands-dispatcher-thread";
    private static final String NOTIFICATION_DISPATCHER_THREAD_PREFIX = "notification-dispatcher-thread";
    ...
    public static void main(String[] args) throws TelegramApiException, IOException, SQLException {
        ...
        final StateRepository stateRepository = KernelInitializer.initStateRepository(config, STATE_DB_FILENAME);
        final ActorRef commandDispatcherActor = KernelInitializer.initKernelCommandDispatcherActor(
        ...
        KernelInitializer.startHandlers(
                ...
                getCommandResultHandlerSupplier(tgMessagesQueue, discordMessagesQueue, commandResultsQueue));
        KernelInitializer.startHandlers(
                config.getNotificationHandlerThreadNumber(),
                NOTIFICATION_DISPATCHER_THREAD_PREFIX,
                getNotificationHandlerSupplier(config, tgMessagesQueue, discordMessagesQueue, notificationsQueue));
        ...
        KernelInitializer.startCheckNotifications(actorSystem, config, stateRepository, notificationsQueue);
        logKernelStarted();
    }
    ...
    private static Supplier<Runnable> getCommandResultHandlerSupplier(
            ...
            final BlockingQueue<ResultCommandContainer> discordMessagesQueue,
            final BlockingQueue<ResultCommandContainer> commandResultsQueue) {
        return () -> new CommandResultRouter(commandResultsQueue, tgMessagesQueue, discordMessagesQueue);
    }
    ...
    private static Supplier<Runnable> getNotificationHandlerSupplier(
            ...
            final BlockingQueue<ResultCommandContainer> discordMessagesQueue,
            final BlockingQueue<NotificationContainer> notificationsQueue) {
        return () -> new NotificationRouter(notificationsQueue, tgMessagesQueue, discordMessagesQueue, config);
    }
