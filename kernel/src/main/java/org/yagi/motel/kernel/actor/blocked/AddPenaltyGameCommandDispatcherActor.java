...
public class AddPenaltyGameCommandDispatcherActor extends AbstractActor {
    ...
    public AddPenaltyGameCommandDispatcherActor(
        ...
        this.portalAddPenaltyGameUrl =
                UrlHelper.normalizeUrl(String.format("%s/api/v0/autobot/add_penalty_game", config.getPortalUrl()));
    }
                        ...
                        switch (message.getType()) {
                            case ADD_PENALTY_GAME:
                                AddPenaltyGameRequest request = AddPenaltyGameRequest.builder()
                                        ...
                                        .gameId(Long.valueOf(
                                                message.getPayload().getMessageValue()))
                                        .build();

                                Optional<AddPenaltyGameResponse> addPenaltyGameResponse = RestClient.sendPost(
                                        mapper,
                                        RestClient.preparePostRequest(portalAddPenaltyGameUrl, request, mapper),
                                ...
                                if (addPenaltyGameResponse.isPresent()) {
                                    commandResultsQueue.put(ResultCommandContainer.builder()
                                            ...
                                            .resultMessage(
                                                    addPenaltyGameResponse.get().getMessage())
