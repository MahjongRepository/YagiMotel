...
public class LogCommandDispatcherActor extends AbstractActor {
    ...
    private void processMajsoulLog(InputCommandMessage message) throws IOException, InterruptedException {
        ...
        if (hashOptional.isPresent() && !StringUtils.isEmpty(hashOptional.get())) {
            ...
            if (replayInfo.isPresent()) {
                Optional<GameFinishRequest> gameFinishRequest = GameFinishRequest.convertFromTensoulMap(
                        config, mapper, replayInfo.get(), message.getRequestedResponseLang());
                if (gameFinishRequest.isPresent()) {
                    // todo handle error
                    Optional<GameFinishResponse> response = RestClient.sendPost(
                            mapper,
                            RestClient.preparePostRequest(portalGameFinishUrl, gameFinishRequest.get(), mapper),
    ...
    private void processTenhouLog(InputCommandMessage message) throws IOException, InterruptedException {
        ...
        Optional<AddGameLogResponse> addGameLogResponse = RestClient.sendPost(
                mapper,
