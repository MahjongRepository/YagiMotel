...
import org.yagi.motel.kernel.actor.blocked.*;
...
public class KernelInitializer {
    ...
    public static StateRepository initStateRepository(final AppConfig config, final String stateDbFilename)
            throws SQLException {
    ...
    public static ActorRef initKernelCommandDispatcherActor(
        ...
        return configureCommandDispatcherActor(actorSystem, config, stateRepository, commandResultsQueue);
    }
    ...
    public static ActorRef initErrorCommandDispatcherActor(
        ...
        return actorSystem.actorOf(
                ErrorCommandDispatcherActor.props(commandResultsQueue).withMailbox(PriorityMailbox.DISPATCHER_NAME),
    ...
    public static <T> void startHandlers(
        ...
        ExecutorService executor =
                Executors.newFixedThreadPool(handlerThreadsNumber, threadFactorySupplier.apply(handlerThreadPrefix));
    ...
    private static void runTgBot(TgTournamentHelper bot, AppConfig config) throws TelegramApiException {
        ...
        startHandlers(config.getTelegram().getMessagesThreadNumber(), TG_MESSAGES_THREAD_PREFIX, () -> bot);
    }
    ...
    private static void updateState(AppConfig config, StateRepository stateRepository) {
        stateRepository.updateProcessingState(
                Boolean.TRUE.equals(config.getDisableOnStart()) ? IsProcessedState.DISABLE : IsProcessedState.ENABLE);
    }
    ...
    private static ActorRef configureCommandDispatcherActor(
        ...
        final ActorRef logCommandDispatcherActor = actorSystem.actorOf(
        ...
        final ActorRef meCommandDispatcherActor = actorSystem.actorOf(
                MeCommandDispatcherActor.props(config, commandResultsQueue)
                        .withRouter(new SmallestMailboxPool(config.getEventsMailboxPool()))
                        .withMailbox(PriorityMailbox.DISPATCHER_NAME),
                MeCommandDispatcherActor.ACTOR_NAME);

        final ActorRef addCommandDispatcherActor = actorSystem.actorOf(
                AddCommandDispatcherActor.props(config, commandResultsQueue)
                        .withRouter(new SmallestMailboxPool(config.getEventsMailboxPool()))
                        .withMailbox(PriorityMailbox.DISPATCHER_NAME),
                AddCommandDispatcherActor.ACTOR_NAME);

        final ActorRef startRegistrationCommandDispatcherActor = actorSystem.actorOf(
        ...
        final ActorRef closeRegistrationCommandDispatcherActor = actorSystem.actorOf(
        ...
        return actorSystem.actorOf(
                CommandDispatcherActor.props(commandDispatchersHolder).withMailbox(PriorityMailbox.DISPATCHER_NAME),
