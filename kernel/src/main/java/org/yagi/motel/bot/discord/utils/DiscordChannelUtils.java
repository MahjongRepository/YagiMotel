...
public class DiscordChannelUtils {
    ...
    public static Optional<Long> getTargetChatIdFromNotificationContainer(
        ...
        Optional<Long> defaultNotificationChatId = getDiscordNotificationChatId(notificationContainer, config);
        String currentMessageType = notificationContainer.getMessageType();
        if (PortalNotificationMessageType.CONFIRMATION_STARTED.getMessageType().equals(currentMessageType)) {
